#include "Vdeterminisation_completion.h"
#include "Projet Automates finis et Expressions rationnelles/determinisation_completion.h"
#include "Projet Automates finis et Expressions rationnelles/test_automates.h"


void determinisation(Automate* AF, const char* nom_fichier_trace_execution){


int nouv_nb_trans=0;
int nouv_nb_etats=0;
int* nouv_etat_depart;
int* nouv_etat_arrivee;
int* nouv_symbole;
int numeroSymbole=0;
int* listEtats;
...
void Vcompletion(Automate* AF, const char* nom_fichier_trace_execution) {

    FILE *fichier = fopen(nom_fichier_trace_execution, "aw");
    fprintf(fichier, "\nAutomate complété, nouvelles transitions : \n");

    char *alphabet[AF->nb_symboles]; //on stocke chaque lettre de l'alphabet de l'automate
    int presente = 0;
    int numsymbole = 0;
    for (int cpt = 0; cpt < AF->nb_transitions; cpt++) {
        int cptTableau = 0;
        while (presente == 0 && cptTableau <=
                                numsymbole) { //tant qu'on a pas trouvé la lettre dans le tableau et qu'on a pas dépassé la taille du tableau utilisé
            if (AF->symboles[cpt] == alphabet[cptTableau])
                presente = 1; //si la transition contient un symbole déjà inscrit on le note
            else
                cptTableau++; //sinon on cherhce jusqu'à la fin du tableau rempli
        }
        if (presente == 0) {//si la lettre est nouvelle
            alphabet[numsymbole] = AF->symboles[cpt]; //on l'ajoute au tableau de l'alphabet
            numsymbole++; // on incremente le compteur de cases utilisés
        }
    }
//maintenant on verifie si chaque transition est presente, si une manque, on ajoute la transition à l'état poubelle
    int nouv_transition = AF->nb_transitions + 1;
    for (int i = 0; i < AF->nb_etats; i++) {//on traite chaque etat
        int cpt = 0;
        char *symbolesPresents[AF->nb_symboles];
        for (int j = 0; j < AF->nb_transitions; j++) {//on compte combien de transitions existent avec cet etat
            if (AF->etats_depart[j] == i) {
                symbolesPresents[cpt] = AF->symboles[j];//on note quel lettre est deja presente
                cpt++;
            }
        }
        if (cpt < AF->nb_symboles) {
            for (int k = 0; k < AF->nb_symboles; k++) {
                //on regarde dans notre premier tableau et on le compare au deuxieme
                int presente = 0;
                int un_autre_cpt = 0;
                while (presente == 0 && un_autre_cpt < AF->nb_symboles) {
                    if (alphabet[k] == symbolesPresents[un_autre_cpt])presente = 1;
                    else
                        un_autre_cpt++;
                }
                if (presente ==
                    0) {//la lettre n'est pas dans une transition, on ajoute une nouvelle transition avec l'état poubelle
                    AF->etats_depart[nouv_transition] = i;
                    AF->symboles[nouv_transition] = alphabet[k];
                    AF->etats_arrivee[nouv_transition] = -1;
                    fprintf(fichier, "\nTransition : %d%c%d\n", AF->etats_depart[nouv_transition],
                            AF->symboles[nouv_transition], AF->etats_arrivee[nouv_transition]);
                    nouv_transition++;
                    AF->nb_transitions++;
                }
            }
        ...
        for (i = 0; i < AF->nb_transitions; i++) {
            ...
            if (AF->symboles[i] == '*') {
                ...
                printf("L'automate contient au moins une transition epsilon : %d%c%d\n", AF->etats_depart[i],
                       AF->symboles[i], AF->etats_arrivee[i]);
...
void elimination_epsilon(Automate *AF, const char *nom_fichier_trace_execution, int numero) {
    ...
    if (fichier != NULL) {
        ...
        if (numero == 31) {
            ...
            AND_synchrone_31(AF, fichier, etat_initial_transition_epsilon, etat_final_transition_epsilon,
                             numero_etat, compteur1);
            ...
            AND_synchrone_32(AF, fichier, etat_initial_transition_epsilon, etat_final_transition_epsilon,
                             numero_etat, compteur1);
            ...
            AND_synchrone_33(AF, fichier, etat_initial_transition_epsilon, etat_final_transition_epsilon,
                             numero_etat, compteur1);
            ...
            AND_synchrone_34(AF, fichier, etat_initial_transition_epsilon, etat_final_transition_epsilon,
                             numero_etat, compteur1);
            ...
            AND_synchrone_35(AF, fichier, etat_initial_transition_epsilon, etat_final_transition_epsilon,
                             numero_etat, compteur1);
...
void AND_synchrone_31(Automate *AF, FILE *fichier, int etat_initial_transition_epsilon[],
                      int etat_final_transition_epsilon[], int numero_etat[], int compteur1) {
...
void AND_synchrone_32(Automate *AF, FILE *fichier, int etat_initial_transition_epsilon[],
                      int etat_final_transition_epsilon[], int numero_etat[], int compteur1) {
...
void AND_synchrone_33(Automate *AF, FILE *fichier, int etat_initial_transition_epsilon[],
                      int etat_final_transition_epsilon[], int numero_etat[], int compteur1) {
...
void AND_synchrone_34(Automate *AF, FILE *fichier, int etat_initial_transition_epsilon[],
                      int etat_final_transition_epsilon[], int numero_etat[], int compteur1) {
...
void AND_synchrone_35(Automate *AF, FILE *fichier, int etat_initial_transition_epsilon[],
                      int etat_final_transition_epsilon[], int numero_etat[], int compteur1) {
        ...
        if (est_un_automate_deterministe(AF, nom_fichier_trace_execution) == true) {
            ...
            while (i <= AF->nb_transitions) {
                ...
                if (nb_de_symbole_different(tab) != AF->nb_symboles) {
                    ...
                    fprintf(fichier,
                            "L'automate n'est pas complet : de chaque etat ne sortent pas toutes les fleches avec toutes les etiquettes possibles\n");
...
void completion2(Automate *AF, const char *nom_fichier_trace_execution) {
        ...
        if (est_un_automate_asynchrone(AF, nom_fichier_trace_execution) == 0) {
            ...
            for (i = 0; i < AF->nb_etats; i++) {
                ...
                for (a = 0; a < nb_symbole; a++) {
                    ...
                    printf("Transition ajoutee : %d%c%c\n", AF->etats_depart[AF->nb_transitions],
                           AF->symboles[AF->nb_transitions], AF->etats_arrivee[AF->nb_transitions]);
